export default {
  async fetch(request, env, ctx) {
    ...
    if (
      ...
      const { results } = await env.DB.prepare(
        "SELECT email, ts FROM signups WHERE campaign = ?1 ORDER BY ts DESC"
      )
        .bind(campaign)
        .all();
  ...
  async scheduled(event, env, ctx) {
    const { results } = await env.DB.prepare(
      "SELECT campaign, email, ts FROM signups WHERE ts >= datetime('now', '-1 day') ORDER BY campaign, ts"
    ).all();
    ...
    for (const row of results) {
      ...
      grouped[row.campaign].push(`- ${row.email} (${row.ts})`);
    }

    let content = "**Daily Signups Report**\n";
